// Simple demonstration of using XQuery from OS X Yosemite Javascript for Applications
...
function run() {
	...
	var dctXQuery = {
		...
		'Pomodoro tags grouped by Area': ' \
			let $v := distinct-values(//outline[@pom]/ancestor::*/body/outline/@posn) \
			for $h in $v \
			let $hdr := //body/outline[@posn=$h], \
				$lines := $hdr//outline[@pom], \
				$title := string(data($hdr[1]/@text)), \
				$fmt := concat(upper-case(substring($title, 1, 1)), substring($title, 2)) \
			return ( \
				concat("## ", $fmt), \
				for $i in $lines \
				let $p := number($i/@pom) \
				order by $p descending \
				return concat("- ", $i/@text, " @pom(", $p, ")") \
			) \
		',

		'Pomodoro tags grouped by Area and Duration': ' \
			let $v := distinct-values(//outline[@pom]/ancestor::*/body/outline/@posn) \
			for $h in $v \
			let $prj := //body/outline[@posn=$h], \
				$lines := $prj//outline[@pom], \
				$title := string(data($prj[1]/@text)), \
				$fmt := concat(upper-case(substring($title, 1, 1)), substring($title, 2)) \
			return ( \
				concat("\n## <font color=silver>", $fmt, "</font>"), \
	...
	var strFolderPath =
		"/Users/robintrew/txtquery-tools/Yosemite Javascript XQuery demo/SampleFolder";

	// SHOW A MENU OF PERSPECTIVES

	while (varChoice) {

		// BUILD AN XINCLUDE DOCUMENT FOR ANY ELIGIBLE FILES IN THE FOLDER
		lstFiles = filesInFolderWithExtn(strFolderPath, strExtn);
		if (lstFiles.length) {
			strXML = xIncludeXML(
				filesInFolderWithExtn(strFolderPath, strExtn), 'combined', 'doc'
			);
		...
		// OFFER A CHOICE OF PERSPECTIVES (XQUERY REPORTS)
		lstMenu = Object.keys(dctXQuery);
		lstMenu.sort(function (a, b) {
			var d = a.length - b.length; // Shorter menu items first
			return d ? d : (Number(a < b) ? -1 : 1); // alpha if same length
		});
		...
		varChoice = lstMenu.length ? app.chooseFromList(lstMenu, {
			...
			defaultItems: (typeof varChoice)[0] === 'b' ? [lstMenu[0]] : varChoice
		}) : false;
		...
		if (i) {
			...
			app.displayAlert(
				"No matches for " + strMenuKey + " found in " +
				strFolderPath + " .opml files"
			);
		}
