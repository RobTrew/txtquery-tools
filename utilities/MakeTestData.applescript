...
function(editor, options) {
		...
		dteNow=new Date(), dteHorizon=new Date(2015, 11, 30), dteStart=dteNow,
		...
		function simplePhrase(lngLevel, dteLocalStart, dteLocalDue) {
			...
			for (j=lngTags; j--;) {
				...
				lstParts.push(['@',strKey,'(',varValue.toString(),')'].join(''));
			}
		...
		tree.beginUpdates();
		phraseTree(tree, oParent, 4,3, 1, dteNow, dteHorizon);
		tree.endUpdates();
