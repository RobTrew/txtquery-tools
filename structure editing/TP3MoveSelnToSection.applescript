...
"property pstrHeadingList : "
	...
	function(editor) {
		var  libNodePath = require('ft/core/nodepath').NodePath,
			oTree = editor.tree(),
			lstHeads = oTree.evaluateNodePath('//@type=project'),
			oLastNode,
			...
			if (lngSeln) {
				oLastNode=lstSelnNodes[lngSeln-1];
				rngLines = oTree.createRangeFromNodes(lstSelnNodes[0],0,oLastNode,oLastNode.line().length);
