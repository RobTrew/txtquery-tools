JsOsaDAS1.001.00bplist00ÑVscript_$á// Translate FTML file (or clipboard contents) to FT / MD text
...
function run() {
	...
	function xmlParse(strHTML) {
		var error = $(),
		oDoc = $.NSXMLDocument.alloc.initWithXMLStringOptionsError(
			strHTML, 0, error
		);
		return error.code ? ObjC.unwrap(error.localizedDescription) : oDoc;
	}
			...
			if (strName === 'head') {
				...
				while (j--) {
					...
					if (ObjC.unwrap(oHeadNode.name) === 'meta') {
						if (ObjC.unwrap(oHeadNode.attributeForName('name').stringValue) === 'expandedItems') {
							dct.meta = oHeadNode;
							break;
						}
					}
	...
	function main() {
		...
		varParse = xmlParse(strHTML);
		if (typeof varParse === 'string')
			{ return "Could not be parsed as FTML\n"; }
